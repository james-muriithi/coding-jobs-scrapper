...
def scrap_jobs(domain):
    ...
    divs = soup.find_all(name="li", attrs={"class": "job-list-li"})
...
def postJob(data):
    ...
    try:
        x = requests.post(endpoint, headers={
                          'Accept': 'application/json', 'Authorization': 'Bearer {}'.format(token)}, json=data)
        print(x.json())
...
def main():
    page1 = 'https://www.myjobmag.co.ke/jobs-by-field/information-technology'
    scrap_jobs(page1)
